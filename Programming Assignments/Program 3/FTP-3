...
try:
    ftp = ftplib.FTP()
    ftp.set_debuglevel(2)  # Show FTP command/response debugging
    ftp.connect(FTP_HOST, FTP_PORT)
    ftp.login(user, password)
    ftp.set_pasv(False)  # Force active mode

    files = []
    print("Connected and logged in. Fetching directory listing...")

    ftp.retrlines('LIST', callback=files.append)  # Use retrlines instead of dir()

    print("Files fetched from FTP:", len(files))
    for f in files:
        print(f)
